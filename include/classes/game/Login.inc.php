	...
	*/
	...
	function register2(& $vars) {
		...
		if (isset ($error))
			...
			$email = "Um Deine Anmeldung zu vervollst�ndigen musst du den folgenden Link anklicken:\n".$link;
			Mailer::simplesend(Setting::read('systememail'), $spieler->get('email'), "Anmeldung f�r Sea Wars", $email);
	...
	function confirm(&$vars) {
		...
		if($spieler->get('confirmed')==1) error("Dieser Spieler wurde bereits verifiziert", 'Login', 'confirm');
		if((isset($vars['hash']) && !empty($vars['hash']))) {
			$hash = $mysql->escape($vars['hash']);
			if($hash != $spieler->get('hash'))
				error("Falscher Link, falsche Security ID �bermittelt.", 'Login', 'confirm');
			...
			error("Falscher Link, keine Security ID �bermittelt.", 'Login', 'confirm');
	}
