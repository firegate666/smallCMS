...
<table class="adminlist" width="100%">
	...
	{
		if (isset($_REQUEST['tpl_class']) && ($items == $_REQUEST['tpl_class']))
			$options .= '<option selected="selected">' . $items . '</option>';
		else
	...
	<tr>
		...
		<td>
			<form name="selectclass" action="index.php" method="get">
				...
				<select name="tpl_class" onChange="this.form.submit()"><?php print $options ?></select>
			</form>
		</td>
		<?php if (isset($_REQUEST['tpl_class']) && !empty($_REQUEST['tpl_class']))
		{ ?>
			<td>Template</td>
			<td>
				<form name="showtemplate" action="index.php" method="get">
					<input type="hidden" name="admin"/>
					<input type="hidden" name="template"/>
					<input type="hidden" name="tpl_class" value="<?php print $_REQUEST['tpl_class'] ?>"/>
					<input type="hidden" name="admin"/>
...
<?php if (isset($_REQUEST['tpl_layout']))
				{ ?>
					<script type="text/javascript" src="?admin/show/javascript"></script>
					<form action="index.php" method="post" name="edittpl">
						<p>Template '<?php print $_REQUEST['tpl_layout'] ?>' bearbeiten</p>
						<input type="submit" name="submit" value="Speichern und schlie&szlig;en"/>
						...
						<table>
							...
							<tr>
								<td>
									<textarea name="tpl_content" cols="80" rows="50"><?php print (Template::getLayout($_REQUEST['tpl_class'], $_REQUEST['tpl_layout'], array(), true, array(), true)); ?></textarea>
								</td>
							</tr>
							<tr>
								<td>
									<select name="contenttype">
<?php print Template::contenttypeoptionlist($_REQUEST['tpl_class'], $_REQUEST['tpl_layout']); ?>
				</select>
			</td>
		</tr>
    </table>
    ...
    <input type="hidden" name="tpl_class" value="<?php print $_REQUEST['tpl_class'] ?>"/>
	<input type="hidden" name="tpl_layout" value="<?php print $_REQUEST['tpl_layout'] ?>"/>
</form>
<?php } ?>
